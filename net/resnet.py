...
class ProjectionLayer(nn.Module):
    def __init__(self, encoder_outdim, proj_hidden_dim, proj_output_dim):
        ...
        self.proj = nn.Sequential(
            nn.Linear(encoder_outdim, proj_hidden_dim),
            nn.ReLU(),
            nn.Linear(proj_hidden_dim, proj_output_dim),
        )


        init.kaiming_normal_(self.proj[0].weight, mode='fan_out')
        if self.proj[0].bias is not None:
            init.constant_(self.proj[0].bias, 0)


        init.kaiming_normal_(self.proj[2].weight, mode='fan_out')
        if self.proj[2].bias is not None:
            init.constant_(self.proj[2].bias, 0)
...
class Resnet18(nn.Module):
    def __init__(self, embedding_size, pretrained=True, is_norm=True, bn_freeze=True):
        ...
        #self.checkpoint = torch.load('./result/cub/resnet1/cub_resnet18_best_step_0.pth')
